...
;@Ahk2Exe-SetName Popup Hotkeys 2
;@Ahk2Exe-SetDescription Popup Hotkeys 2
;@Ahk2Exe-SetVersion 1.8 BETA
;@Ahk2Exe-SetCopyright Jean Lalonde
;@Ahk2Exe-SetOrigFilename PopupHotkeys2.exe
...
strResult :=  CreatePopupHotkeys(blnWithReport, intLaunchDelay)
if (strResult <> "")
...
; ------------------------------------------------
{
	global blnWithReport
	global intLaunchDelay
...
;@Ahk2Exe-IgnoreEnd
	...
	if !FileExist(strIniFileName)
		...
		(LTrim
			...
			[Keys]
			; Key#=name | hotkey | excutable_path | working_directory | no_preload | window_identifier | startup_script
...
; ------------------------------------------------
CreatePopupHotkeys(blnDisplayReport := true, intLaunchDelay := 10)
; ------------------------------------------------
